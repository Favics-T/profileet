      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { icon: MessageCircle, label: 'Reviews', value: reviews.length > 0 ? String(reviews.length) : '0' },
        ...
        {items.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">{items.length} piece{items.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3">
              {items.map(item => {
                const tag = TAG_COLORS[item.tag] ?? TAG_COLORS.Other
                return (
                  <div key={item.id} className="relative aspect-square rounded-xl overflow-hidden group bg-gray-100">
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt={item.title}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {r.reply && (
                        <div className="mt-2 pl-3 border-l-2 border-[#FF6500]/30">
                          <p className="text-xs text-gray-500 italic">"{r.reply}"</p>
                        </div>
                      )}
                    </div>
                  </div>
